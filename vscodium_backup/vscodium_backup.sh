...
BACKUP_DIR="$HOME/.config/VSCodium/Backup_VSCodium"
...
# Criar o pacote de backup incluindo configurações e lista de extensões, excluindo workspaceStorage e History
tar --exclude="globalStorage" \
--exclude="History" \
--exclude="workspaceStorage" \
-czvf "$BACKUP_DIR/$BACKUP_FILE" \
-C "$CONFIG_DIR" . \
-C "$BACKUP_DIR" "$EXTENSIONS_FILE"
